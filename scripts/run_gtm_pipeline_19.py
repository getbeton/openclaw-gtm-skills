...
# ── Config (loaded from local config files — never hardcode secrets) ──────────
_base = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
_cfg = json.loads(open(os.path.join(_base, "config", "config.local.json")).read())
SUPABASE_URL = _cfg["supabaseUrl"]
SUPABASE_KEY = _cfg["supabaseKey"]
FIRECRAWL_ENDPOINT = _cfg.get("firecrawlUrl", "http://localhost:3002")
APOLLO_KEY = json.loads(open(os.path.join(_base, "integrations", "apollo.json")).read())["api_key"]
