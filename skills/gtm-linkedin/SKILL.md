...
## Firecrawl config
- Endpoint: configured via `firecrawlUrl` in `openclaw.plugin.json`
- Self-hosted: see https://github.com/mendableai/firecrawl for setup (optionally with Soax or similar residential proxies for LinkedIn)
...
## Run it
```bash
cd /path/to/openclaw-gtm-skills/scripts
python3 run_linkedin.py --limit 1000 --workers 5 --min-employees 10 --max-employees 5000
```

## Dependencies
- Firecrawl at the URL configured in `openclaw.plugin.json`
