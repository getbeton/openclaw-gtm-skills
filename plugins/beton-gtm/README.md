# Beton GTM Intelligence Plugin

Outbound prospecting pipeline for [Beton](https://getbeton.ai). Bundles all GTM intelligence skills into a single plugin that takes a list of domains and turns them into researched, scored, segmented, and outreach-ready prospects.

## What it does

1. **Intake** — normalizes domains, deduplicates against Supabase
2. **Pre-filter** — checks if domain resolves, LinkedIn exists, employee range is plausible
3. **Research** — Firecrawl deep research → classification JSONB (B2B, SaaS, GTM motion, vertical, pricing model)
4. **Sales Org** — careers + LinkedIn jobs → headcount estimates, open roles, tech stack signals
5. **Signals** — news, LinkedIn posts, G2 reviews → urgency-scored signals (last 90 days)
6. **Segment** — matches company profile against active ICP segments → fit_score + fit_tier
7. **Outreach** — drafts personalized email sequences per experiment config → queues for Vlad review
8. **Deck** — triggered on positive reply → generates sales deck via sales-deck skill

## Pipeline

```
domains.csv
    │
    ▼
[gtm-intake]       → dedup, normalize
    │
    ▼
[gtm-prefilter]    → parallel x10, skip bad domains
    │
    ▼
[gtm-research]     → parallel x5, deep Firecrawl crawl
    │
    ▼
[gtm-sales-org]    → parallel x5, headcount + tech stack
    │
    ▼
[gtm-signals]      → parallel x10, news + LinkedIn + G2
    │
    ▼
[gtm-segment]      → batch, fit score + tier
    │
    ▼
Supabase + Attio sync
    │
    ▼
[gtm-outreach]     → if experiment_id, draft sequences for T1
    │
    ▼
Vlad reviews → seqd enrollment
    │
    ▼
[gtm-deck]         → triggered on replied_positive
```

## Configuration

Set in OpenClaw plugin config or environment:

| Key | Description |
|-----|-------------|
| `supabaseUrl` | Supabase project URL |
| `supabaseKey` | Supabase service key |
| `attioApiKey` | Attio API key |
| `firecrawlUrl` | Local Firecrawl instance (default: `http://localhost:3002`) |
| `seqdApiUrl` | seqd email sequencer API (default: `http://localhost:8000`) |

## Running the pipeline

```bash
cd /home/nadyyym/.openclaw/workspace/plugins/beton-gtm/scripts

# Full pipeline on a CSV
python3 pipeline.py --domains path/to/domains.csv

# With experiment
python3 pipeline.py --domains path/to/domains.csv --experiment-id <uuid>

# Resume interrupted run
python3 pipeline.py --domains path/to/domains.csv --resume
```

## Schema Design Principles

1. **Flat over JSONB.** Prefer flat relational tables with FKs over nesting data in JSONB columns. JSONB is only acceptable for truly unstructured/variable data (raw Firecrawl page content, signal content, email sequence drafts). Everything with a known structure gets its own table and columns.

2. **Migrate more, not less.** More tables = simpler analytics. It's fine to add a table or column via a new migration rather than stuffing things into existing JSONB blobs.

3. **Always use Supabase CLI migrations.** Never apply schema changes by executing raw SQL directly. Every schema change must be a migration file:
   ```bash
   # Create migration
   supabase migration new <descriptive_name> --workdir /home/nadyyym/.openclaw/workspace/plugins/beton-gtm
   # Edit the generated file in supabase/migrations/
   # Apply to remote
   supabase db push --workdir /home/nadyyym/.openclaw/workspace/plugins/beton-gtm --password <pw>
   ```
   Migration files live in `supabase/migrations/` and are the single source of truth for schema state.

## Schema

See `supabase/migrations/` for the full schema history. Tables:
- `segments` — ICP definitions with hypotheses
- `companies` — researched prospects with all JSONB blobs
- `contacts` — decision-makers
- `signals` — urgency-scored triggers
- `experiments` — A/B test tracking
- `hypotheses` — testable claims linked to experiments
- `experiment_hypotheses` — junction table
- `outreach` — drafted sequences
- `results` — outcomes per outreach
- `learnings` — post-experiment summaries

## Dependencies

- Firecrawl running at `http://localhost:3002`
- Soax proxies configured at `integrations/soax.json`
- Supabase project with schema applied
- Attio workspace with known attribute slugs (see `scripts/attio.py`)
- seqd running at `http://localhost:8000` (for enrollment)
- Existing `sales-deck` skill at `~/.openclaw/workspace/skills/sales-deck/`
